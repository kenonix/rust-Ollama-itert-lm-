...
impl LitProcess {
    ...
    async fn spawn_with_backend(
        ...
        let python_code = format!(
            r#"import pty, os, sys, termios, select
...
if pid == 0:
    ...
    try:
        while True:
            r, w, x = select.select([sys.stdin.fileno(), fd], [], [])
                ...
                loop {
                    match stdout.read(temp_buf).await {
                        ...
                        Ok(n) => {
                            ...
                            tracing::info!("Raw read from process stdout: {:?}", text);

                            if !seen_newline {
                                if let Some(newline_pos) = text.find('\n') {
                                    seen_newline = true;
                                    let remaining = text[newline_pos + 1..].to_string();
                                    buffer.clear();
                            ...
                            if let Some(pos) = text.find(">>>") {
                                ...
                                // Send the final chunk (without the >>> and trailing newlines/spaces)
                                let final_text = text[..pos].trim_end_matches('\n');
                            ...
                            if text.len() > last_chunk.len() {
                                ...
                                if !new_content.is_empty() {
                                    ...
                                    // Update last_chunk only up to what we actually sent!
                                    let sent_len = last_chunk.len() + new_content.len();
                                    last_chunk = text[..sent_len].to_string();
                                }
                            }
                        }
                        Err(e) => {
